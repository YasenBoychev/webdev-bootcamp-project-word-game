// This component renders heart images in the status bar based on the number of lives left
...
export default function Lives() {
  ...
  const initialLives = useSelector(state => state.gameplay.initialLives);
  const lives = useSelector(state => state.gameplay.lives);

  let heartsRender = [];
  for (let i = 0; i < lives; i++) {
    heartsRender.push(
      <div key={`fullHeart${i + 1}`} className="heart">
  ...
  for (let i = 0; i < (initialLives - lives); i++) {
    heartsRender.push(
      <div key={`emptyHeart${i + 1}`} className="heart">
